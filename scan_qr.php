...
<body class="admin-page">
    ...
    <script>
    let globalID = null, html5QrCode = null, currentMode = 'camera';

    function extractAppointmentId(text) {
        try { const j = JSON.parse(text); if (j.id) return j.id; } catch {}
        const m = text.match(/\d+/);
        return m ? m[0] : null;
    }

    function showAppointmentModal(a) {
        const date = new Date(a.date).toLocaleDateString('en-PH', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    ...
    document.querySelectorAll('#statusButtons button').forEach(btn => {
        btn.addEventListener('click', function() { if (globalID) updateStatus(globalID, this.dataset.status); });
    });
    ...
    document.getElementById('fileInput').addEventListener('change', function(e) {
        ...
        html5QrCode.scanFile(file, true).then(text => {
            ...
            if (!id) { Swal.fire('Invalid', 'No valid ID found.', 'warning'); return; }
            globalID = id;
    ...
    function startScanner() {
        ...
        Html5Qrcode.getCameras().then(cameras => {
            if (cameras?.length) {
                html5QrCode.start(cameras[0].id, { fps: 10, qrbox: 250 }, text => {
                    const id = extractAppointmentId(text);
                    if (!id) return;
                    globalID = id;
                    html5QrCode.pause();
                    ...
                    }).catch(() => { Swal.fire('Error', 'Fetch failed.', 'error'); html5QrCode.resume(); });
                }, () => {});
